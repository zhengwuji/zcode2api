...
#### 🚀 新增特性 & 核心升级
* **🌟 网页端实时展示当前正在使用的账号 (Active Account Visibility)**：
  * **主选活动横幅 (Active Account Banner)**：页面顶部新增当前正在生效的主选账号横幅，配备呼吸发光脉冲灯（`Pulse Glow`），实时展示账号头像、邮箱/手机号/昵称、平台渠道（Z.AI 国际 / 智谱国内）、凭证类型与额度概况。
  * **表格行高亮聚焦**：当前生效账号在明细列表中以浅绿发光底色（`row-active-current`）与翡翠绿左边框醒目突出，并附带绿色发光徽章 `[● 当前正在使用]`。
  * **一键设为主选**：支持在 Web 后台对任意正常可用账号点击【设为主选】，秒级激活并同步到底层代理引擎与配置。
* **⚡ 自动切号前端实时无感自动更新 (Zero-F5 Live Sync)**：
  * 前端引入 3 秒静默定时轮询机制，后台或网关因网络闪断、502 中断或额度耗尽自动轮换到新账号时，网页端无需用户手动 F5 刷新，界面自动更新高亮状态并弹出切换提示通知 Toast。
* **⚠️ 被切换账号原因可追溯记录与展示 (Traceable Switch Reasons)**：
  * **全链路原因捕获**：网关层与自愈守护进程精准捕获并持久化账号被换下的具体原因，涵盖：
    * `上游连接中断 (502 Bad Gateway)`
    * `模型额度已耗尽 (0/3.0M)`
    * `上游请求频次限流 (HTTP 429)`
    * `流首包读取异常 / 网络连接异常`
    * `上游凭证已失效 (HTTP 401)`
    * `后台 / 控制台手动切换`
  * **行内警示条展示**：被切账号在身份下方以醒目的黄色警示盒呈现 `⚠️ 被切原因: ... (切换时间)`，排查问题与额度消耗一目了然。
...
### [v2.3.0] - 2026-09-26
* **🟢 实时 Token 吐字监控与速率透明化**：
  * **流式生成动态追踪**：SSE 流式调用时动态在控制台同一行实时刷新生成的 Token 数量与实时吐字速率（如 `🟢 输出中: 128 tokens (65.2 tok/s)`）。
  * **请求完成精准统计**：调用结束时精准输出调用总 Token 数、耗时与平均速度（如 `✔ 200 OK 流式完成 (共 127 tokens · 耗时 2.52s · 50.4 tok/s)`），并兼容 OpenAI 与 Anthropic 官方 `usage` 自动校准。
  * **请求账号直观可视**：每笔请求前置清晰打印当前处理该请求的具体账号（如 `账号: 17684811785 (Z.AI)`），不再盲目黑屏。
* **⚡ 纯静默自愈进程管理**：
  * 彻底移除所有可能误伤 PID 0 的逻辑，绝无 `taskkill /f /pid 0` 刷屏与 cmd 命令回显杂音。
  * Watchdog 智能识别流式输出状态，长文本生成中绝不误判假死重启。
* **💓 存活心跳与多模式运行选择**：
  * 网关启动即输出包含监听地址、生效账号、当前额度的大气运行看板。
  * 空闲时每 30 秒自动输出存活心跳行（`🟢 网关运行正常 | 监听: 8080 | 状态: 就绪待命`），彻底告别“不知道服务挂了还是活着”的焦虑。
  * 控制台提供多种运行模式自由切换：`[1]` 自愈网关（实时 Token 监控 + 自动秒切自愈）、`[P]` 调试原生代理、`[U]` 原生交互式终端。
